...
const Inventory = ({ onUpdateInventory }) => {
  const initialItems = [
    { id: 1, name: 'Pine Wood', quantity: 500, location: 'Warehouse A', status: 'In Stock', type: 'Wood', description: 'Good for framing and general construction' },
    { id: 2, name: 'Steel Beams', quantity: 120, location: 'Warehouse B', status: 'In Stock', type: 'Steel', description: 'Used for structural support' },
    { id: 3, name: 'Concrete Blocks', quantity: 0, location: 'Warehouse C', status: 'Out of Stock', type: 'Concrete', description: 'Used for building walls' },
    { id: 4, name: 'Oak Wood', quantity: 150, location: 'Warehouse A', status: 'In Stock', type: 'Wood', description: 'Durable wood for furniture and flooring' },
    { id: 5, name: 'Rebar', quantity: 200, location: 'Warehouse B', status: 'In Stock', type: 'Steel', description: 'Reinforcement bars for concrete' },
    { id: 6, name: 'Cement Bags', quantity: 350, location: 'Warehouse C', status: 'In Stock', type: 'Concrete', description: 'Portland cement for construction' },
    { id: 7, name: 'Aluminum Sheets', quantity: 75, location: 'Warehouse D', status: 'In Stock', type: 'Metal', description: 'Lightweight metal sheets for roofing' },
    { id: 8, name: 'Birch Plywood', quantity: 200, location: 'Warehouse A', status: 'In Stock', type: 'Wood', description: 'Plywood sheets for cabinetry' },
    { id: 9, name: 'Galvanized Pipes', quantity: 100, location: 'Warehouse E', status: 'In Stock', type: 'Steel', description: 'Corrosion-resistant pipes for plumbing' },
    { id: 10, name: 'Sand', quantity: 1000, location: 'Warehouse C', status: 'In Stock', type: 'Aggregate', description: 'Fine sand for concrete mix' },
    { id: 11, name: 'Gravel', quantity: 500, location: 'Warehouse C', status: 'In Stock', type: 'Aggregate', description: 'Coarse aggregate for concrete mix' },
    { id: 12, name: 'Redwood Lumber', quantity: 50, location: 'Warehouse A', status: 'Out of Stock', type: 'Wood', description: 'High-quality wood for decking' },
  ];
  ...
  const addItem = (newItem) => {
    ...
    if (onUpdateInventory) onUpdateInventory(newItemWithId);
  };

  const editItem = (updatedItem) => {
    setItems(items.map(item => (item.id === updatedItem.id ? updatedItem : item)));
    if (onUpdateInventory) onUpdateInventory(updatedItem);
    setEditingItem(null);
  };
  ...
  const filteredItems = items.filter(item => {
    ...
    const matchesTypeFilter = materialTypeFilter === 'All' || item.type === materialTypeFilter;
    return matchesSearchTerm && matchesFilter && matchesTypeFilter;
  });
...
const InventoryForm = ({ item = {}, onSave, onCancel }) => {
  ...
  const [type, setType] = useState(item?.type || 'Wood');
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSave({ ...item, name, quantity, location, status, type, description });
...
const OrderManagement = () => {
  const [orders, setOrders] = useState([
    {
      id: '000000003',
      purchasePoint: 'Main Website Store',
      purchaseDate: 'Dec 26, 2016, 9:12:00 AM',
      billToName: 'Gabriel Guarino',
      shipToName: 'Gabriel Guarino',
      grandTotal: '$25.00',
      status: 'Pending'
    },
    {
      id: '000000002',
      purchasePoint: 'Main Website Store',
      purchaseDate: 'Dec 15, 2016, 9:12:00 AM',
      billToName: 'Veronica Costello',
      shipToName: 'Veronica Costello',
      grandTotal: '$39.64',
      status: 'Complete'
    },
    {
  ...
  const handleDownloadBill = (order) => {
    ...
    doc.text(`Purchase Point: ${order.purchasePoint}`, 10, 20);
    doc.text(`Purchase Date: ${order.purchaseDate}`, 10, 30);
    doc.text(`Bill-to Name: ${order.billToName}`, 10, 40);
    doc.text(`Ship-to Name: ${order.shipToName}`, 10, 50);
    doc.text(`Grand Total: ${order.grandTotal}`, 10, 60);
  ...
  const filteredOrders = orders.filter(order =>
    ...
    order.purchasePoint.toLowerCase().includes(searchTerm.toLowerCase()) ||
    order.billToName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    order.shipToName.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="order-management">
        ...
        <tbody>
          {filteredOrders.map(order => (
            <tr key={order.id}>
              ...
              <td>{order.status}</td>
              <td>
                <button onClick={() => handleDownloadBill(order)}>Download Bill</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <Modal className="modal" isOpen={modalIsOpen} onRequestClose={closeModal} contentLabel="Create New Order">
  ...
  <form onSubmit={handleSubmit}>
    <label>
      Order ID:
      <input type="text" name="id" value={newOrder.id} onChange={handleChange} required />
    </label>
    <label>
      Purchase Point:
      <input type="text" name="purchasePoint" value={newOrder.purchasePoint} onChange={handleChange} required />
    </label>
    <label>
      Purchase Date:
      <input type="text" name="purchaseDate" value={newOrder.purchaseDate} onChange={handleChange} required />
    </label>
    <label>
      Bill-to Name:
      <input type="text" name="billToName" value={newOrder.billToName} onChange={handleChange} required />
    </label>
    <label>
      Ship-to Name:
      <input type="text" name="shipToName" value={newOrder.shipToName} onChange={handleChange} required />
    </label>
    <label>
      Grand Total:
      <input type="text" name="grandTotal" value={newOrder.grandTotal} onChange={handleChange} required />
    </label>
    <label>
      Status:
      <input type="text" name="status" value={newOrder.status} onChange={handleChange} required />
    </label>
